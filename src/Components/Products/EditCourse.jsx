...
const EditCourse = ({ show, onClose, dataCourse, reCharge }) => {
    ...
    const onSubmit = handleSubmit(data => {
        console.log(data);
        dispatch(editCourseThunk(data.id, data))
            .then(() => {
                onClose();
                reCharge();
                reset();
    ...
    return (
        <div>
            <Dialog header="Editar Curso" visible={show} style={{ width: '380px', height: '85vh' }} onHide={() => onClose()} footer={footerContent}>
                <form style={{ width: '100%' }} onSubmit={onSubmit} >
                    ...
                    <div style={{ marginBottom: '5px', width: '100%' }}>
                        <label htmlFor="linkCourse" style={{
                            display: 'block', marginBottom: '5px', fontWeight: 'bold'
                        }}>Link del curso</label>
                        <InputText id="linkCourse" type='text'
                            style={{ width: '100%' }}
                            className={`p-inputtext-sm ${errors.name ? 'p-invalid' : ''}`}
                            {...register("linkCourse",
                                {
                        ...
                        {errors.linkCourse && <span style={{ color: 'red', fontSize: '15px' }}>{errors.linkCourse.message}</span>}
                    </div>
                </form>
            </Dialog>
        </div>
